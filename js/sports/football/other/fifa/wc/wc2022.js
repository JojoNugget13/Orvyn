...
function renderMatches(selectedMatchday = 'all') {
    ...
    let filteredMatches = wc2022.matches;
    if (selectedMatchday !== 'all') {
        filteredMatches = wc2022.matches.filter(md => md.matchday === selectedMatchday);
    }
    ...
    if (filteredMatches.length === 0) {
        ...
        md.games.forEach(game => {
            ...
            html += `
                <a href="wc2022/${game.id}.html" class="match-card-link">
...
function initMatchdayFilter() {
    ...
    // FIXED: uclData2425 -> wc2022
    const matchdays = ['all', ...new Set(wc2022.matches.map(md => md.matchday))];
...
function renderRegularSeasonStandings(container) {
    const legendItems = [
        { status: 'qualified',  label: 'Advances to Round of 16', color: '#10b981' },
    ...
    // Loop over each group (A, B, C, ...) and build its own mini table
    Object.entries(wc2022.standings).forEach(([groupLetter, teams]) => {
        html += `
            <div class="group-block">
                <div class="group-block-title">Group ${groupLetter}</div>
                <div class="standings-wrapper" style="border: none; border-radius: 0;">
        ...
        teams.forEach(team => {
            ...
            html += `
                <tr class="status-${team.status}">
                    ...
                    <td class="team-col">${teamContent}</td>
                    <td>${team.p}</td>
                    <td>${team.w}</td>
                    <td>${team.d}</td>
                    <td>${team.l}</td>
...
function renderPlayerStats() {
    ...
    const categories = [
        { name: 'Attacking', data: wc2022.playerStats.attacking },
        { name: 'Defending', data: wc2022.playerStats.defending },
        { name: 'Goalkeeping', data: wc2022.playerStats.goalkeeping }
    ];
...
function renderTeamStats() {
    ...
    wc2022.teamStats.forEach(group => {
        html += `<div class="stats-category-group">
